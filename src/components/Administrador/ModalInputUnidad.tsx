...
const ModalInputUnidad: React.FC<ModalInputProps> = ({ isOpen, onClose, onAdd, message, isAcceptAction, esHijo = false, idPadre }) => {
  const [nuevaUnidad, setNuevaUnidad] = useState<UnidadDerivacion>(unidadInicial);
  const [isOpenModal, setIsOpenModel] = useState<boolean>(false);
  const [isOpenModalSuccess, setIsOpenModalSuccess] = useState<boolean>(false);
  const [isOpenModalError, setIsOpenModalError] = useState<boolean>(false);
  const { createUnidad, createSubUnidad } = useUnidadDerivacion();
  const handleOnAddAgregarUnidad = (nuevaUnidad: UnidadDerivacion) => {
    if(esHijo){
      createSubUnidad(nuevaUnidad).then((result) => {
        if (result) {
      ...
      createUnidad(nuevaUnidad).then((result) => {
        if (result) {
          setIsOpenModalSuccess(true);
        }
        else {
    ...
    <Transition.Root show={isOpen} as={Fragment}>
      <Dialog as="div" className="fixed z-10 inset-0 overflow-y-auto" onClose={onClose}>
              ...
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <div className="text-center">
                  <Dialog.Title as="h1" className="text-2xl leading-6 font-semibold text-gray-900">
                    Nueva {esHijo ? `SubUnidad` : `Unidad`} de Derivación
                  </Dialog.Title>
                  <InputAdmin2 
        ...
        <ModalSuccess 
          ...
          message={`Se creó con éxito la Unidad: ${nuevaUnidad && nuevaUnidad.nombre}`}
          onClose={() => {
...
export default ModalInputUnidad;