...
const InputTutor = ({ titulo = "", texto = "", enable = true, name, value, onChange, readOnly, manejarBlur }: InputProps) => {
  return (
    <li className="drop-shadow-md p-3 flex items-center space-x-5 height-100% personalizado"
      ...
      <input type="text"
        className={`w-full h-full px-3 py-2 mt-1 font-roboto text-base bg-[rgba(255,_255,_255,_0.50)] border-custom drop-shadow-md font-normal ${readOnly ? 'text-gray-700' : ''}`}
        style={{ marginLeft: "0" }}
        placeholder={texto}
        disabled={!enable}
        name={name}
