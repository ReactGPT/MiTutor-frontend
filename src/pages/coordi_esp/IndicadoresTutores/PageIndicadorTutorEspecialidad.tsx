...
import { Button, Combobox, Dropdown, SearchInput } from '../../../components';
...
const PageIndicadorTutorEspecialidad: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      let url = `${ServicesProperties.BaseUrl}/listarTutoresConCantidadDeProgramasEspecialidad/${selectedIdEspecialidad}`;
  ...
  const fetchDataAppointment = async () => {
    try {
      let url = `${ServicesProperties.BaseUrl}/listarCantidadAppointmentsEspecialidad/${selectedIdEspecialidad}`;
      if (startDate && endDate) {
        url += `?startDate=${startDate}&endDate=${endDate}`;
      }
  ...
  const handleExportClick = async () => {
    if (data.length > 0) {
      ...
      doc.setFontSize(20);
      doc.setFont('helvetica', 'bold');
      doc.text('FICHA DE INDICADORES TUTORES - FACULTAD', 105, 10, { align: 'center' });
      doc.setFontSize(8);
    ...
    try {
      ...
      if (roles) {
        ...
        if (iFaculty !== -1 && userData?.userInfo?.roles[iFaculty].details && 'departmentName' in userData.userInfo.roles[iFaculty].details) {
          ...
          const responseIds = await api.get<{ success: boolean, data: number[]; }>(`/listarTutoresPorIdEspecialidad/${selectedIdEspecialidad}`);
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4">
      <div className="flex justify-between items-end h-fit">
        ...
        <button className="min-h-[40px] bg-primary cursor-default rounded-xl text-white px-5 shadow-custom border border-solid border-[rgba(116,170,255,0.70)] active:bg-black hover:cursor-pointer ml-6" onClick={handleBuscarTutoresClick}>
          Buscar Tutores
        </button>
