...
import { Faculty, ManagerRoleDetails, ProgramaTutoria, Tutor } from '../../../store/types';
...
export default function PageProgramasTutoriaMaestroEsp() {
  ...
  const handleNavigation = (data: ProgramaTutoria) => {
    //console.log(data);
    navigate("/programasDeTutoriaMaestroEsp/editar", { state: { programaTutoria: data } });
  };
  ...
  const programaTutoriaFiltered: ProgramaTutoria[] = useMemo(() => {
    ...
    if (roles) {
      programaTutoriaData.forEach(programa => {
        ...
        roles.forEach(role => {
          if (role.rolName === 'Responsable de Especialidad') {
            ...
            //&& !filteredData.some(prog => prog.especialidadId == facultyId)
            if (programa.especialidadId === facultyId && !filteredData.some(prog => prog.id == programa.id)) {
      ...
      if (filteredData.length === 0) {
        ...
        programaTutoriaData.forEach(programa => {
          if (programa.especialidadId === facultyId) {
            filteredData.push(programa);
          }
        });
      }
