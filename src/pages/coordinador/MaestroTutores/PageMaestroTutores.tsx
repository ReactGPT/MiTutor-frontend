...
function PageMaestroTutores() {
    ...
    return (
    <div className='flex flex-col w-full h-full gap-2 px-4'>
        <div className='flex flex-row w-full h-[5%] justify-between'>
        <h2 className='text-xl font-bold text-primary'>Maestro de Tutores</h2>
            {isLoadingPostTutores?
                        <Spinner/>:
                        <Button text='Guardar' 
                                icon={SaveIcon} 
                                onClick={()=>{postTutores(tutoresModified)
                                                .then((success)=>{
                                                    if(success){
        ...
        <TutoresTable className='flex w-full h-[50%] items-center justify-center'
                        ...
                        />
        <div className='flex flex-row w-full h-[5%] justify-between'>
            <div className='flex flex-row h-full max-h-[80px] w-[150px] gap-2 mt-2 border-custom shadow-custom '>
                <button className='flex h-full w-[50%] text-primary items-center justify-center' onClick={()=>handleOnSelectUsuariosView('Usuarios')}>
                    Usuarios
                </button>
                <button className='flex h-full w-[50%] text-primary items-center justify-center' onClick={()=>handleOnSelectUsuariosView('Alumnos')}>
                    Alumnos
                </button>
            </div>
        </div>
        <div className='flex w-full h-[45%]'>
            ...
            <UsuariosTable className='flex w-full h-full flex-col mt-2' 
                            usuarios={usersToShowTable}
                                handleAddUser={handleOnAddUser}
                                />:
            <AlumnosTable className='flex w-full h-full flex-col mt-2' 
...
export default PageMaestroTutores