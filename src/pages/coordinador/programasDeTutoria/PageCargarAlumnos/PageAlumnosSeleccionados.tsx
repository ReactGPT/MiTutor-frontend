...
const PageAlumnosSeleccionados = ({
  isOpen,
  closeModal,
}: ModalAlumnosSeleccionadosProps) => {
  const navigate = useNavigate();
  const { tutoringProgram: tutoringProgramSelected } =
  ...
  const [estudiantesAEliminar, setestudiantesAEliminar] = useState<Student[]>([]);
  // Efecto para cargar datos originales cuando se abre el modal
  useEffect(() => {
    if (
      isOpen &&
      tutoringProgramSelected.alumnos &&
      tutoringProgramSelected.alumnos.length > 0
    ) {
  ...
  const handleTutorChange = (studentId: number, tutorId: number) => {
    const updatedStudentData = studentDataModified.map((student) => {
      if (student.studentId === studentId) {
        const selectedTutor = tutoringProgramSelected.tutores.find(
          (tutor) => tutor.idTutor === tutorId
        );
        return {
          ...student,
  ...
  const handleOnClickDeleteIcon = () => {
    if (estudiantesAEliminar.length > 0) {
        const newAlumnos = [...(studentDataModified.filter((estudiante) => !estudiantesAEliminar.some((item) => item.studentId === estudiante.studentId)))];
        setStudentDataModified(newAlumnos);
        setestudiantesAEliminar([]);
    }
};
  ...
  const columnDefs: ColDef[] = !esFijo
    ? [
        ...
        { headerName: "Facultades", field: "facultyName", minWidth: 150 },
        {
          headerComponent: () => {
              return (
                  <button className='text-primary' onClick={() => { if (estudiantesAEliminar.length > 0) { setIsOpenDelete(true); } }}><DeleteIcon /></button>
              );
          },
          maxWidth: 60,
          minWidth: 40,
          cellRenderer: (row: any) => {
              //console.log(row);
              return (
                  <Checkbox value={estudiantesAEliminar.some(item => item.studentId === row.data.studentId)}
                      onChange={() => {
                          const studentIsFound = estudiantesAEliminar.some(item => item.studentId === row.data.studentId);
                          if (studentIsFound) {
                              setestudiantesAEliminar([...(estudiantesAEliminar.filter(tutor => tutor.studentId !== row.data.studentId))]);
                          } else {
                              setestudiantesAEliminar([...estudiantesAEliminar, row.data]);
                          }
                      }}
                  />
              );
    ...
    : [
        ...
        { headerName: "Facultades", field: "facultyName", minWidth: 150 },
        {
          headerName: "Tutor Fijo",
          field: "tutorFijo",
          minWidth: 150,
          cellRenderer: (params: any) => {
            const student = params.data;
            const tutorExistsInProfessors =
              tutoringProgramSelected.tutores.some(
                (tutor) => tutor.idTutor === student.tutorId
              );
            return (
              <select
                ...
                onChange={(e) =>
                  handleTutorChange(student.studentId, parseInt(e.target.value))
                }
              >
  ...
  const handleClosePageCargarAlumno = useCallback(() => {
    // Restaurar studentDataModified a los datos originales si es necesario
    setStudentDataModified([...tutoringProgramSelected.alumnos]);
    setshowPageCargarAlumno(false);
    closeModal();
  }, [setshowPageCargarAlumno, tutoringProgramSelected.alumnos]);
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              <Dialog.Panel className="max-w-screen-xl w-2/3 min-h-[50%] max-h-[80%] bg-white transform rounded-2xl p-6 shadow-xl transition-all">
                <Dialog.Title
                  as="div"
                  className="flex flex-row h-[10%] max-h-[80px] items-center ml-2 mr-2 justify-begin mb-[20px]"
                >
                  ...
                  {showPageCargarMasivamente ? (
                    ...
                    <div>
                      ...
                      <div className="w-full h-[50%]">
                        <div
                          className="flex w-full h-[75%] mt-6 ag-theme-alpine ag-theme-alpine2"
                          style={{ height: "300px" }}
                        >
                          <div className="w-full h-full">
                            <AgGridReact
                              ...
                              rowData={studentDataModified.filter(
                                (student) =>
                                  student.name
                                    .toLowerCase()
                        ...
                        <div className="flex justify-end mt-6 gap-[10px]">
                          <Button
                            onClick={handleClosePageCargarMasivamente}
                            text="Cerrar"
                          />
                          <Button
                            onClick={handleClickSaveAlumnos}
                            icon={SaveIcon}
                            text="Guardar"
                            iconSize={8}
                          />
                      ...
                      <ModalConfirmation
                        isOpen={isOpenDelete}
                        isAcceptAction={true}
                        message="¿Esta seguro de eliminar estos alumnos?"
                        onClose={() => {
