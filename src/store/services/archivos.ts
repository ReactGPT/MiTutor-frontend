...
import { Archivo, ArchivoStudent, FileBD, ExtendedFile } from '../types/Archivo';
...
async function enviarArchivoAlumnoBD(archivo: ArchivoStudent): Promise<number> {
  try {
    const response = await axios.post(ServicesProperties.BaseUrl+'/insertarArchivoAlumnoBD', archivo); 
    console.log('Archivo enviado correctamente a la BD:', response.data);
    return response.data.idArchivo; 
  } catch (error) {
    console.error('Error al enviar el archivo a la BD:', error);
...
//listarArchivosAlumnoBD
async function listarArchivosAlumnoBD(idAlumno: number): Promise<ExtendedFile[]> {
  try {
    const response = await axios.get(`${ServicesProperties.BaseUrl}/listarArchivosAlumno/${idAlumno}`);
    const archivosData = response.data.data;
...
export { enviarArchivo,enviarArchivoAlumnoBD, enviarArchivoBD,listarArchivosBD,descargarArchivo,listarArchivosAlumnoBD};